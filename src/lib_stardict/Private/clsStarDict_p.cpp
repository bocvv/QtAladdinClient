...
inline const gchar *offset_index::read_first_on_page_key(glong page_idx)
{
    ...
    fread(wordentry_buf, std::min(sizeof(wordentry_buf), (unsigned long)page_size), 1, idxfile);
...
//===================================================================
clsSDictPrivate::clsSDictPrivate(progress_func_t f)
{
    progress_func=f;
    iMaxFuzzyDistance  = MAX_FUZZY_DISTANCE; //need to read from cfg.
}

clsSDictPrivate::~clsSDictPrivate()
{	
...
void clsSDictPrivate::SimpleLookup(const std::string &str, SearchResultList_t& res_list)
{
    glong ind;
    ...
    for (gint idict=0; idict<ndicts(); ++idict)
        if (SimpleLookupWord(str.c_str(), ind, idict))
            res_list.push_back(
                        stuSearchResult(dict_name(idict),
                                        poGetWord(ind, idict),
                                        parse_data(poGetWordData(ind, idict))));

}

void clsSDictPrivate::load_dict(const std::string& url)
{
    Dict *lib=new Dict;
    if (lib->load(url)){
        oLib.push_back(lib);
        std::cerr<<"Loaded dic from: "<< url<<std::endl;
    }
    else
    {
...
class DictLoader {
public:
    DictLoader(clsSDictPrivate& lib_): lib(lib_) {}
...
void clsSDictPrivate::load(const strlist_t& dicts_dirs,
...
public:
    DictReLoader(std::vector<Dict *> &p, std::vector<Dict *> &f,
                 clsSDictPrivate& lib_) : prev(p), future(f), lib(lib_)
    {
    }
    void operator()(const std::string& url, bool disable) {
        if (!disable) {
...
void clsSDictPrivate::reload(const strlist_t& dicts_dirs,
                             const strlist_t& order_list,
                             const strlist_t& disable_list)
{
...
const gchar *clsSDictPrivate::poGetCurrentWord(glong * iCurrent)
{
...
const gchar *
clsSDictPrivate::poGetNextWord(const gchar *sWord, glong *iCurrent)
{
    // the input can be:
...
const gchar *
clsSDictPrivate::poGetPreWord(glong * iCurrent)
{
...
bool clsSDictPrivate::LookupSimilarWord(const gchar* sWord, glong & iWordIndex, int iLib)
{
    glong iIndex;
    bool bFound=false;
    gchar *casestr;
...
bool clsSDictPrivate::SimpleLookupWord(const gchar* sWord, glong & iWordIndex, int iLib)
{
...
bool clsSDictPrivate::LookupWithFuzzy(const gchar *sWord, gchar *reslist[], gint distlist[], gint reslist_size)
{
    if (sWord[0] == '\0')
        return false;
    ...
    for (gint i=0; i<reslist_size; ++i) {
        ...
        distlist[i] = oFuzzystruct[i].iMatchWordDistance;
    }
...
gint clsSDictPrivate::LookupWithRule(const gchar *word, gchar **ppMatchWord)
{	
...
bool clsSDictPrivate::LookupData(const gchar *sWord, std::vector<gchar *> *reslist)
{
